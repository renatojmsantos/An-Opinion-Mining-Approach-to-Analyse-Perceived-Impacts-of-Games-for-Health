...
def getCommentsSentiment():
	...
	try:
		...
		#query = "select originaltext, commentid from comment where originaltext LIKE 'I love it song best day ever is me brooks 🤩🤩%' or originaltext LIKE '%I am truly sorry, and now I want to turn away from my past sinful life toward you. Please forgive me, and help me avoid sinning again. %' or originaltext LIKE '%trop cool%' or originaltext LIKE '%western wilds season 3 please! do it like before 2020 and 2019 just dance please%'   or originaltext LIKE '%ISAAC CRUZ FRIEND HAPPY BIRTHDAY%' or originaltext LIKE '%Check out this cool android app on Google Play : Gangnam Style Whip%' or originaltext LIKE '%Dont forget to follow me:%' or originaltext LIKE '%God is a spoon%' or originaltext LIKE '%Re R . I . P . Cameron Boyce ! ?%' or originaltext LIKE '%BLACKPINK IN YOUR AREA%' or originaltext LIKE '%Necesito el marcador local de los 5 mejores de vuelta%' or originaltext LIKE '%MERRY CHRISTMAS%' or originaltext LIKE '%Please, react to the new music video%' or originaltext LIKE '%BEEP BEEP IM A SHEEP%' "
		
		#query = "select originaltext from comment where commentid = 'UgyEPGyp90B9g3pzt0N4AaABAg'"
		...
		query = "SELECT comment.*, annotation.* FROM comment FULL JOIN annotation ON (comment.commentid = annotation.comment_commentid) WHERE comment.commentid IS NULL OR annotation.comment_commentid IS NULL order by length(commentid)"
